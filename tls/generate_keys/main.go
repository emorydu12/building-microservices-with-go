...
func main() {
	...
	_, _ = generateKeyAndCertificate(
		...
		"instance_cert.pem",
		"password",
		applicationKey, applicationCert,
	)
}
...
func generateKeyAndCertificate(
	...
	saveX509Certificate(certData, certificateOutputLocation)
...
func savePrivateKey(key *rsa.PrivateKey, path string, password []byte) error {
	...
	if err != nil {
		return fmt.Errorf("unable to open key.pem for writing: %v", err)
	}
